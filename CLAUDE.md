...
- **Quality and reliability roadmap**: `docs/QUALITY_RELIABILITY_ROADMAP.md` — **ACTIVE**, shared Codex/Claude execution board for desktop delivery, worker recovery, honest diagnostics, startup/UI defects, complete test gates, native quality, feeds, and installed-main release proof. Work must claim a `REL-NNN` task through a draft PR; prediction-adjacent tasks also obey the `ACC-NNN` tracker.
