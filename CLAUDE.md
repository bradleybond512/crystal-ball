...
- **Prediction uplift plan**: `docs/PREDICTION_UPLIFT_PLAN.md` — **ACTIVE** 13-PR program across 4 workstreams: close dark calibration wiring (both prediction bridges have zero live call sites), ground-truth resolver expansion (market/weather/conflict outcome resolvers), correlation engine v-next (inhibitory edges, multi-hop confounder control, Hawkes-lite base rates, per-regime reliability), and measurement gates (bench:correlation CI gate before any engine change, tunable kernel weights last). The doc contains its own Progress Tracker + Session Protocol — implementing sessions must read it first and update the tracker in the same commit.
