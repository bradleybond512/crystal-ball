...
- `npm run doctor:claude-auth` runs `scripts/claude-auth-doctor.mjs` to diagnose a Claude Code CLI that will not stay logged in: config ownership (the `sudo claude` footgun), cloud-sync symlinks, permissions, free disk, overriding `ANTHROPIC_*` env vars and their rc-file source line, and duplicate installs. Rules live in the pure `scripts/claude-auth-core.mjs` (tested via `npm run test:diagnostics`). Read-only by default; `--fix` deletes only leftover `~/.claude.json.tmp*` scratch files. It never invokes `security(1)` — the macOS keychain item is reported as a manual check, per the keychain prohibition at the top of this file.
