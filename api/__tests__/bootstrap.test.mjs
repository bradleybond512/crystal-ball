/**
 * bootstrap.js characterization tests
...
try {
  ...
  console.warn('Handler bootstrap.js failed to import:', err.message);
  handler = null;
}

test('bootstrap.js: rejects unsupported methods', async () => {
  if (!handler) return;
...
test('bootstrap.js: handles missing query params gracefully', async () => {
  if (!handler) return;
