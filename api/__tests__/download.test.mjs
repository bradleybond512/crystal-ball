/**
 * download.js characterization tests
...
let handler;
try {
  handler = (await import('../download.js')).default
    ?? (await import('../download.js'));
} catch (err) {
  console.warn('Handler download.js failed to import:', err.message);
  handler = null;
}

test('download.js: rejects unsupported methods', async () => {
  if (!handler) return;
...
test('download.js: handles missing query params gracefully', async () => {
  if (!handler) return;
