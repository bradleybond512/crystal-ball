/**
 * story.js characterization tests
...
let handler;
try {
  handler = (await import('../story.js')).default
    ?? (await import('../story.js'));
} catch (err) {
  console.warn('Handler story.js failed to import:', err.message);
  handler = null;
}

test('story.js: rejects unsupported methods', async () => {
  if (!handler) return;
...
test('story.js: handles missing query params gracefully', async () => {
  if (!handler) return;
