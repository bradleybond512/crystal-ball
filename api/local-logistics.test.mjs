...
test('local logistics route rejects missing coordinates', async () => {
 ...
 headers: { origin: 'https://crystalball.app' },
  }));
...
test('local logistics route maps Overpass elements into logistics nodes', async () => {
 ...
 headers: { 'content-type': 'application/json' },
  ...
  try {
 const response = await handler(new Request('https://crystalball.app/api/local-logistics?lat=35.994&lon=-78.8986&categories=hospital&limitPerCategory=1', {
 headers: { origin: 'https://crystalball.app' },
 }));
