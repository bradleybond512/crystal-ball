...
| 6 | UI wiring | ✅ | `claude/crystal-ball-improvements-yo8kf7` | Partial slice landed: superforecast-state.ts (on-demand entry point — 15-min signature cache, in-flight dedupe, injectable deps) + AnalystHUD "∑ Superforecast" button/expandable block (hidden in Ghost Mode, copies the simulate-button pattern) + buildSuperforecastLines() in forecast-provenance-view.ts. This also ACTIVATES PR 13's superforecast-vs-baseline run at runtime: requestSuperforecast() pushes live-vs-shadow pairs (pushSuperforecastPair + persistVerdictSnapshot) so cognition:shadow-report finally gets real data. Ask-the-Data wired into Command Center via insights/ask-context.ts in the same PR. Wave 5a surfacing slice (`claude/design-polish-wave5`): cognition-settings.ts kill-switches (5 typed keys, fail-safe ON, localStorage `crystalball-cognition-flags-v2`) + Settings → General → Cognition section (5 toggles, off-behavior descriptions); EVOI surfaced via evoi-surface.ts (provider report + collection gaps context) → AnalystHUD "What to check next" section (top-5, +bits badges, panel jump, delegated clicks); episodic analogs surfaced per-hypothesis ("Historical analogs" disclosure, title/date/sim%, detail line); BOCPD surfaced via regime-scan.ts + regime-monitor.ts (60s throttle, per-domain dedupe) → TriageBar amber regime chip + elevated toast; consolidation surfaced via consolidation-state.ts (persisted report + Run now) → CrisisSignaturePanel status block. Every service entry point (recall/recordEpisode, planCollection surface, regime scan+read, cadence+manual run, shadow push×3) gates on its switch. Wave 5b polish slice (`claude/design-polish-wave5`): at-a-glance SummaryStrip above the panels grid includes a gated active-regime-shift segment (reads getActiveRegimeShifts(), hidden when the bocpd switch is off, clicks through to the Analyst HUD) + Settings → General → Overview kill toggle. Calibration report card SHIPPED (`claude/cognition-calibration-report`): pure `cognition/calibration-report-view.ts` — `buildCalibrationReportCard(records)` composes PR 2's `buildCurve` (reliability sparkline: predicted vs PAV-repaired observed per bin) + per-domain Brier + signed calibration gap into a `CalibrationReportCard` (per-domain rows + pooled global row + overall verdict); reliability label = well_calibrated / overconfident ("runs hot") / underconfident ("runs cold") / insufficient_data, with a display floor MIN_REPORT_N=12 distinct from the MIN_DOMAIN_N/MIN_GLOBAL_N curve-activation gates, and `curveSource` (domain→global→identity) mirroring `getRecalibrator()`'s fallback ladder so a user sees whether recalibration is actively engaged. Surfaced in Command Center via `renderCalibrationReport()` (reads `getCalibrationStore().all()`, renders nothing until ≥1 forecast resolves — honest empty state). Tests: calibration-report-view.test.mts (8: empty/well/hot/cold/below-floor/curve-ladder/sparkline/sort) added to test:cognition (598/598). Typecheck 0. Both formerly-deferred PR 6 leftovers now SHIPPED via PR #1499 (commit 1e68cc0b, "Prediction Uplift PR A3"): (a) EVOI question-suggester re-ranking — `question-suggester.ts` imports `buildCheckNextItems` from `cognition/evoi-surface.ts` and merges EVOI info-gain actions (`+bits` badges) ahead of the heuristic questions, deduped/ranked/capped; (b) pushRecalibrationPair wiring — `hypothesis-forecast.ts` calls `maybePushRecalibrationPair(signatureFor(h), calibratedP, rawProbability, Date.now())` at the live `forecastHypothesis()` site, with a per-signature 1h flood cap (`RECAL_PUSH_INTERVAL_MS`, `lastRecalPush` map + size-500 eviction) that decouples push frequency from HUD render frequency — closing the "render-path flood risk" that caused the deferral. PR 6 is now ✅ complete. Earlier slice tests: superforecast-state.test.mts (6) + forecast-provenance-view superforecast lines (4). |
...
| 14 | Compute Placement + Hygiene | 🔄 | `claude/cognition-pr14-perf` | Implemented 4 of the plan's items; deferred 1 with justification, 1 was already shipped pre-PR-14 (see PR 14 section note in the spec above). (1) `idle-scheduler.ts`: pure `scheduleIdleWork(task, opts)` — `requestIdleCallback` with a `setTimeout(0)` fallback (guard-unref'd per the reasoning-metrics pattern), skips entirely while `document.visibilityState === 'hidden'` (visibility guard — next cadence tick retries, nothing dropped forever); every dependency injectable. Wired into `consolidation-cadence.ts`: the tick logic was split out into an exported, fully-injectable `runConsolidationTick(opts)` (was inline in the `setInterval` callback) so consolidation's heavy clustering pass now runs via idle scheduling instead of synchronously on the timer tick — satisfies "Consolidation (PR 8) schedules via requestIdleCallback with a visibility guard" without a full Worker offload (see spec note: `vector-index.ts` already documents brute-force top-K as intentionally sub-millisecond at the 2,000-episode cap; a Worker hop's postMessage structured-clone cost on a multi-MB Float32Array corpus would be a regression, not an optimization, and would break Node-test-runner purity — `cognition.worker.ts` was not built). (2) Embedding middle tier (quantized ONNX model spike): deferred — explicitly framed as a spike in the plan, requires sourcing/vetting a real ~25 MB model binary under `public/`, out of scope for a hygiene PR. (3) `embedding-cache.ts`: content-hash → vector memo (FNV-1a-style 2×32-bit hash, non-cryptographic, collision-tolerant), cap 5,000 with insertion-order LRU eviction, persisted to `reasoning_memory` (IDB) + localStorage bootstrap mirror following the loaded/writtenSinceLoad guard pattern; wraps `embed()` and is injectable (`embedFn`) for tests. Wired into `episodic-memory.recall()`, `recallWithContext()`, and `recordEpisode()` — the last one specifically fixes a real pre-existing waste: `recordEpisode()`'s signature-dedupe check runs *after* `embed()` is awaited, so a standing pending hypothesis was re-embedding its own summary every ~5-minute analyst cycle only to be discarded by the dedupe match; now that's a cache hit. Documented trade-off: a cache hit freezes whichever tier resolved at write time (doesn't interact with `maybeUpgradeEmbedding()`'s per-episode tier-upgrade path, which is unaffected). (4) Memory hygiene — episodic dedupe by signature before insert: already shipped pre-PR-14 (no change). (5) Memory hygiene — contradiction flagging: `Episode.contradicted?: { reason, markedAt }` (optional, backward-compatible); `markEpisodeContradictory(episodeId, reason)` and `contradictEpisodesForRefutation(ctx)` in `episodic-memory.ts`; `analogScoreFor()` excludes contradicted episodes from the supportive weighted average (and from the `MIN_RECALLS_FOR_ANALOG` count) while `recall()`/`buildExplanation()` keep surfacing them with `flagged contradictory: <reason>` in the explanation string — contradictions surface, never silently dropped. Real wiring: `situation-hypothesis-bridge.ts` gained an injectable `onHypothesisRefuted` callback (new `BridgeOptions` field, `RefutedHypothesisEvent` type) fired once per non-leading hypothesis exactly at the existing `engine.updateStatus(..., 'refuted')` call site, carrying real `situationId`/`domain`/`entityIds`/`claim`/`hypothesisType` — kept the intelligence-layer bridge free of any dependency on the cognition layer built on top of it (house layering). `panel-layout.ts` wires `onHypothesisRefuted` to `contradictEpisodesForRefutation`, which matches on case-insensitive entity overlap (capped at 10 episodes per refutation event, most-recent-first). Documented, not hidden: `situation-store-v2`'s `entityIds` (free-form slugs like `'suez-canal'`) and whatever a given episode producer supplies in `entities` are two independently-evolved vocabularies with no guaranteed overlap today (analyst-loop's only current producer hardcodes `entities: []`), so this bridge is real, tested, and cheap to call unconditionally but will rarely fire in production until a producer aligns its `entities` field — no code changes needed here when that alignment lands. Tests: `idle-scheduler.test.mts` (6: rIC path, custom timeout, hidden-skips-entirely, setTimeout fallback + unref, unref-less timer tolerance, default-visible-in-Node), `embedding-cache.test.mts` (9: miss calls embedFn, identical-text-never-re-embedded, independent entries per text, returned-vector-is-a-copy, LRU touch, storage persistence, hash determinism/distinctness, tier+dim round-trip), `episodic-memory.test.mts` +14 (contradictory flag/idempotent-first-reason-wins/unknown-id, recall-still-retrievable-with-flag-in-explanation, analogScoreFor excludes contradicted from both the average and the MIN_RECALLS_FOR_ANALOG count, contradictEpisodesForRefutation entity-match/no-match/cap/no-double-flag), `consolidation-cadence.test.mts` +6 (`runConsolidationTick` Ghost/kill-switch/not-due skip, idle-scheduled run + persists last-run, hidden-tab skip, never-throws-on-rejection), `situation-hypothesis-bridge.test.mts` +3 (`onHypothesisRefuted` fires once per non-leader on consensus with real context, throwing hook doesn't break the ledger recorder, no-callback path unchanged). 38 new tests across 5 files; 2 new files (`idle-scheduler.test.mts`, `embedding-cache.test.mts`) added to `test:cognition`. Typecheck 0 errors. `test:cognition` 562/562, `test:intelligence` 430/430, full `test:renderer` (536 files, 11,877 tests) 0 failures in ~58s. |
...
| 16 | Cognition Benchmark + CI Gate | 🔄 | `claude/cognition-pr16-bench` | Implemented per Part D spec — last PR in the stack. `cognition/__bench__/golden-windows.ts`: 12 frozen fixture windows (house pattern: `ops/replay-fixtures-catalog.ts`) spanning conflict/markets/cyber/weather/shortage(macro)/maritime/aviation/general, each with a HypothesisLike descriptor, a fixed ground-truth outcome, a deterministic model-forecast stand-in, and 5 hand-built episodic Recall fixtures; plus a 4-cluster/24-episode held-out training corpus (one-hot vectors, exact `consolidation.test.mts` pattern) and a 360-record synthetic-but-well-calibrated `CALIBRATION_POOL` (fixed-seed mulberry32 PRNG, n≥45/domain — clears both recalibration's n≥30 and conformal's n≥40 domain floors). `cognition/bench-cognition.ts`: `runCognitionBenchmark()` replays every window through the real production functions (matchReferenceClass → blendWithEpisodic → aggregate → buildCurve/recalibrate → conformalInterval), skipping only the LLM-gated decomposition/persona stages — PR 3's own deterministic-only floor (base rate + episodic + model-forecast) is exactly what a fully-offline CI gate must exercise. Schema stage calls the real `runConsolidation()` (fully injectable per its own test pattern: `storage: null`, stub IDB fns, stub registrar, `resetConsolidationForTests()` first for repeatable runs) over the held-out training corpus, then matches schemas to windows by domain and grades materializationRate≥0.5 predictions against actual outcomes — deliberately includes one miscalibrated cluster (cyber, trained low) so the metric is a real (0.75) measurement, not a rigged 100%. Prints Brier, conformal coverage rate, analog-recall precision@5, schema true-positive rate, p50/p95 latency. `cognition/bench-baseline.ts` + committed `bench-baseline.json`: pure comparison mirroring `ops/replay-baseline.ts` exactly — fails on Brier regression >0.02 absolute or conformal coverage dropping >5pt below target, or an unacknowledged window-count drift; update only deliberately, in a reviewed diff. `scripts/cognition-benchmark.mts` (`npm run bench:cognition`) is the CLI/CI entry point, wired as a new step in `.github/workflows/smoke.yml` after `test:renderer`. Fully offline, fixed-seed, no LLM calls, no timers — runs in single-digit milliseconds (12 windows), cannot hang or slow CI. Tests: `bench-cognition.test.mts` (17: one result per window, non-empty explanation chains, universal reference-class match, run-to-run determinism, metric bounds, hand-verified schemaTruePositiveRate=0.75 fixture value, injectable custom windows incl. unmatched-domain and thin-analog edge cases, no-hang timing assertion, baseline-comparison pass/fail/multi-regression/window-drift cases) added to `test:cognition` (581/581 green). Typecheck 0 errors. `npm run test:renderer` (full suite, 1082 suites / 11,896 tests) 0 failures in ~59s — confirms the standing regression guard from the PR 14/military-surge timer-hang incident holds. Spec-vs-code note: the plan's "schema true-positive rate" metric had no formula specified (unlike Brier/coverage, which do); implemented as recall (TP / actual positives) over windows a learned schema matched by domain, restricted to actual-positive windows so a "no signal" unmatched window is reported separately (`schemaMatchedCount`/`schemaTotalCount`) rather than silently folded into the rate — documented in `golden-windows.ts` and `bench-cognition.ts` headers. **Plan status: with this PR's merge, all 16 PRs of the Cognitive Enhancement Plan are implemented — see "Plan status: COMPLETE" note below.** |
...
(reliability curve sparkline + Brier per domain) — SHIPPED via
`cognition/calibration-report-view.ts` + `CommandCenterPanel.renderCalibrationReport()`
(a live per-domain Brier *trend* still wants a persisted history series; the
current card shows the current-window Brier + reliability-diagram sparkline) —
and dossier-driven watch items;
...
- **Worker offload**: vector top-K over 2 000 × 768-dim and consolidation
  ...
  > **Spec-vs-code note (PR 14 implementation session):** `vector-index.ts`'s
  > own header comment documents the top-K scan as intentionally
  > brute-force and sub-millisecond at the 2,000-episode cap ("no ANN
  > library is introduced; this module has zero new runtime dependencies")
  > — a Worker hop would add postMessage structured-clone cost on a
  > multi-MB Float32Array corpus that exceeds the compute it's replacing,
  > and would break the module's pure-Node-testable invariant. A literal
  > `cognition.worker.ts` was not built; the `requestIdleCallback` +
  > visibility-guard half of this bullet *was* implemented (see below) as
  > the mechanism that actually matches the current measured cost profile.
- **Embedding middle tier**: `ml-worker.ts` already runs ONNX models
  ...
  > Deferred: this bullet is explicitly framed as a spike, not a shipped
  > deliverable, and requires sourcing/vetting a real ~25 MB model binary
  > under `public/` — out of scope for a hygiene PR and not something an
  > implementation session can responsibly fabricate.
- **Embedding cache**: content-hash → vector memo in `reasoning_memory`
  (cap 5 000) so re-embedding identical text never happens.
  Implemented in `cognition/embedding-cache.ts`; wired into
  `episodic-memory.recall()` / `recallWithContext()` / `recordEpisode()`.
- **Memory hygiene**: episodic dedupe by signature before insert;
  ...
  > Spec-vs-code note: signature-before-insert dedupe was already shipped
  > pre-PR-14 (`recordEpisode()`'s existing pending-signature check). The
  > refutation-flagging half was net-new in this PR — see the PR 14
  > tracker row for the real wiring and its documented match-rate caveat.
...
**Update (PR 16 implementation session):** as of `claude/cognition-pr16-bench`,
all 16 PRs in this plan have been implemented (Progress Tracker above — PR 16
is 🔄 pending merge, the orchestrator flips it to ✅). This is the last PR in
the stack; there is no PR 17. Future sessions: do not add new PRs here — open
a successor plan once `bench:cognition` has accumulated real production data
to learn from, per the paragraph above.
