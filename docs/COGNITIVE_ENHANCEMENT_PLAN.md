...
| 6 | UI wiring | ✅ | `claude/cognition-pr6-ui` | stacked on cognition-test-fixes. ui-helpers.ts (flags, wipe helpers, 12 formatters); AnalystHUD gains Past analogs block (async recall, outcome badges, similarity %, contradictory flag), Forecast provenance (on-demand superforecast button, estimates table, interval whisker, spread label, llmTier, recalibration explanation), Operator journal ("Your call" 0–99% input + Log button → logForecast, you-vs-system Brier line), EVOI "What to check next" chips (top-3, info-gain bits, tooltip, panel jump). CommandCenterPanel: dossier-enriched Watch Next (getHotEntities, trajectory arrows ▲▬▼, heat%, tooltips) + Calibration report card (buildCurve summary, Brier + miscalibrated bin sentence). Cognition settings section in HUD overlay: 3 feature toggles (episodic-memory / personalization / superforecast, persist crystalball-cognition-flags-v1), 3 destructive actions with 2-click confirm (wipe episodic memory, reset journal, reset operator model). macos-native.css extended with 100+ lines scoped to body.is-desktop-macos + prefers-reduced-motion. ui-helpers.test.mts: 35 new tests covering all formatters + flag API. test:cognition: 614/614 pass. typecheck:all: 0 errors. secrets:scan:staged: clean. PR 14 markEpisodeContradictory hook: confirmed no clean injection point without restructuring competitive-hypothesis.ts — deferral note stands. |
...
| 11 | Change-Point + Semantic Ask-the-Data | 🔲 | `claude/cognition-pr11-stretch` | stretch |
| 12 | Self-Tuning Cognition | ✅ | `claude/cognition-pr12-self-tuning` | stacked on PR10; 8 tunable knobs declared in tunable-params-store (minSim, analogBlendConstant, shrinkagePrior, extremizationK, spreadSkipThreshold, heatHalfLifeHours, interestDecayHalfLifeDays, consolidationClusterThreshold); cognition modules read from store at call time (test purity: injectable override or Node.js localStorage-absent → default); cognition-algorithms.ts registers 5 algorithms (episodic-analog/medium, cognition-recalibration/medium, superforecast/medium, operator-ranking/low, entity-trajectory/medium) + recordEpisodicAnalogEvaluation / recordSuperforecastEvaluation / recordSuperforecastOutcome hooks; drift-detector + safe-adjustment cover cognition via ledger without modification (confirmed: algorithms registered → evaluations flow through shared ledger singleton); self-tuning.test.mts: all 8 knob defaults, bounds clamping, 3 observable behavior changes (minSim/extremizationK/interestDecayHalfLifeDays), registration smoke test, evaluation hook push. PR 11 intentionally skipped (stretch). Typecheck 0 errors. |
...
| 14 | Compute Placement + Hygiene | ✅ | `claude/cognition-pr14-perf` | Stacked on PR13. Worker: cognition.worker.ts + cognition-worker.ts (topK + cluster off main thread, same ?worker pattern as ml-worker.ts, in-thread fallback path for Node.js/tests — identical results guaranteed). Embed cache: content-hash (djb2 normalized text) → vector in reasoning_memory crystalball-cognition-embed-cache-v1, cap 5000 LRU-ish (evict oldest-accessed), tier-upgrade miss (hashed cache entry → miss when neural available), wired into embed() transparently. Memory hygiene: dedupe by signature+kind within 24 h (update existing episode summary if changed, no duplicate insert); markEpisodeContradictory(id|signature) entry point (contradictory flag, reason, persisted); recall() propagates contradictory flag on results; analogScoreFor() excludes contradictory episodes by default (excludeContradictory:true), pass false to include — contradictions surface, never silently dropped (plan invariant). Competitive-hypothesis hook exposed via markEpisodeContradictory; wiring lands with PR 6/12 (no clean injection point without restructuring competitive-hypothesis.ts). ONNX spike: docs/cognition-onnx-embedding-spike.md (deferred — 24 MB first-load cost, three vector spaces operational risk, bundle headroom unconfirmed). Tests: cognition-worker.test.mts (fallback=identical, tier partition, cluster boundary, k limit, minSim); embed-cache.test.mts (hit/miss/tier-upgrade-miss/LRU eviction, determinism invariants); memory-hygiene.test.mts (dedupe window math, kind/signature combos, contradictory marking, analogScoreFor excludes/includes, plan invariant: contradictory episodes retrievable). 3 new test files added to test:cognition. scheduleConsolidation() already has requestIdleCallback + visibility guard from PR8 — no change needed. Typecheck 0 errors. Tests require darwin esbuild (sandbox Linux cannot run). Test fix (claude/cognition-test-fixes): summary-drift assertion corrected — dedupe DOES update the existing episode summary (test had asserted wrong direction; implementation was correct per spec). |
| 15 | LLM Quality Engineering | ✅ | `claude/cognition-pr15-llm-quality` | Stacked on PR14. llm-json.ts: parseStrictJson<T>(text, validate) — direct parse → one repair (strip markdown fences + bracket-matched outermost extraction) → validate → null; extractOutermostJsonBlock uses bracket-count (not regex) to handle nested objects and strings-with-brackets correctly. Refactor: decomposition.ts tryParseJson delegated to parseStrictJson (backward-compat export retained for existing tests); superforecast.ts parsePersonaProbability replaced with parseStrictJson + isPersonaResponse type guard. Self-consistency: medianOf() helper (lower-middle for even arrays, non-mutating); persona elicitation draws k samples (tunable 'cognition-self-consistency-k' bounds 1–5 default 3, declared in tunable-params-store); k=1 is byte-identical to pre-PR path; each sample is budget-checked; on partial budget uses median of succeeded samples. Difficulty routing: decomposition and persona calls do NOT set preferCloud (local-first); only buildAggregateReviewPrompt call sets preferCloud:true; encoded in superforecast.ts, not the adapter. Aggregate review: optional budget-gated final step (one cloud-tier call reviewing aggregate for blunders); JSON {keep, adjustedP?, reason}; applied only within ±0.10 hard clamp (documented + tested); skipped when no persona estimates succeeded. GenerateTextFn extended with preferCloud? for type compatibility. Prompt builders buildPersonaPrompt and buildAggregateReviewPrompt exported for fixture tests. Tests: llm-json.test.mts (direct parse, fence repair, bracket extraction, validator rejection, garbage, no infinite loops, extractOutermostJsonBlock with nested/string brackets); prompt-fixtures.test.mts (all 3 prompt builders: evidence wrapping, JSON contract, section order, balanced tags, no raw feed text outside evidence); self-consistency.test.mts (medianOf odd/even/single/non-mutating, applyAggregateReview keep/adjust/clamp/NaN, k=1 call count + no-note-in-explanation, k=3 call count + median + explanation note, partial-budget degradation, complete-failure degradation). 3 new test files added to test:cognition. Typecheck 0 errors. Tests require darwin esbuild (sandbox Linux cannot run). Test fix (claude/cognition-test-fixes): two bugs corrected — (1) globalThis.localStorage stub added so setTunedParam/getTunedParam work without real browser storage (k was always reading as default 3 in isolation); (2) mock review-call detector tightened from prompt.includes('reviewer') to prompt.includes('senior forecasting reviewer') — the skeptic persona prompt ('skeptical reviewer') was being misrouted as a review call, causing 0 skeptic counts in k=3 test and 6 instead of 3 persona counts in k=1 test. |
| 16 | Cognition Benchmark + CI Gate | ✅ | `claude/cognition-pr16-bench` | Stacked on PR15. golden-windows.ts (14 frozen fixture windows: conflict×3, market×2, weather×3, cyber×2, shortage-proxy×2, aviation×1, macro×1; each seeds episodic memory, provides planted analog signatures, prediction records ≥45 for recalibration+conformal curves, known realized outcome). run-benchmark.ts (pure deterministic runner: seed episodic memory via configureForTests+embedHashed, recall, analogScoreFor, matchReferenceClass+blendWithEpisodic, aggregate deterministic estimates, buildCurve+recalibrate, conformalInterval, runConsolidation with lowered thresholds for hashed-tier bench; BenchmarkReport with overallBrier/coverageRate/analogPrecisionMean/schemaTruePositiveRate/latencyP50P95). scripts/cognition-bench.mjs + npm run bench:cognition (tsx runner, table output, baseline comparison gate: fail on Brier regression >0.02 or coverage <0.75) + bench:cognition:update-baseline. baseline.json with {"pending":true} sentinel (bench exits 0 and instructs update-baseline). CI: typecheck.yml extended with cognition-bench job (needs: typecheck, runs npm run bench:cognition). benchmark.test.mts (runner determinism: two runs identical scores; gate math: +0.02 boundary; coverage gate; pending-baseline path; metric sanity Brier/coverage/precision in [0,1]; catalog: 14 windows, distinct IDs, ≥40 prediction records, domain coverage). test:cognition updated with benchmark.test.mts. Typecheck 0 errors. Stacked on PR15 branch. |
...
4. **Safe adjustment loop**: below-floor cognition algorithms get bounded
   parameter proposals via `safe-adjustment.ts`, validated by
   `backtest-engine.ts` against cognition fixtures before apply. The
   operator approves; nothing self-applies. **The system now tunes its own
   cognition within declared safe envelopes** — this is the single most
   advanced property in the entire plan, and it costs almost no new code.
