...
| **IndexedDB** | `crystalball_db` | Client-side storage for playback snapshots and temporal baseline data. |
...
Crystal Ball ships as three product variants from a single codebase. Each variant surfaces a different subset of panels, map layers, and data sources.

| Variant | Domain | Focus |
|---|---|---|
| `full` | crystalball.app | Geopolitics, military, OSINT, conflicts, markets |
| `tech` | tech.crystalball.app | AI/ML, startups, cybersecurity, developer tools |
| `finance` | finance.crystalball.app | Markets, trading, central banks, macro indicators |
...
```
localStorage('crystalball-variant')  →  import.meta.env.VITE_VARIANT  →  default 'full'
```
...
export const SITE_VARIANT: string = (() => {
 ...
 const stored = localStorage.getItem('crystalball-variant');
...
graph TD
 ...
 UserPref["localStorage overrides<br/>(crystalball-layers)"]
...
2. **User localStorage** — Stored under the key `crystalball-layers`. Users toggle layers in the map controls UI, and their preferences persist across sessions.
...
Crystal Ball employs a five-tier caching strategy to minimise API costs, reduce latency, and enable offline operation.
...
graph TD
 ...
 LSFallback["localStorage fallback<br/>prefix: crystalball-persistent-cache:"]
 PC --> TauriInvoke
 PC --> LSFallback
 end
...
The `crystalball_db` IndexedDB database contains two object stores:
...
**2. Feature Toggles** — 14 `RuntimeFeatureId` values stored in localStorage under the key `crystalball-runtime-feature-toggles`:
...
Crystal Ball runs machine-learning inference directly in the browser using ONNX Runtime Web via Transformers.js, with API-based fallbacks for constrained devices.
...
Crystal Ball uses a circuit-breaker pattern to manage transient failures across its many data sources, preventing cascade failures and providing graceful degradation.
