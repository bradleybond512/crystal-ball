...
> **Crystal Ball** — Config-driven panel architecture powering three site variants.
>
...
4. [Full Variant Panels](#4-full-variant-panels-crystalballio)
5. [Tech Variant Panels](#5-tech-variant-panels-techcrystalballio)
6. [Finance Variant Panels](#6-finance-variant-panels-financecrystalballio)
...
Crystal Ball uses a **config-driven panel system** where every dashboard tile — from live news feeds to AI insights to market data — is declared as a `PanelConfig` entry inside a variant-specific configuration object. The system is designed around three principles:
...
|---------|--------|-------|-------------|
| `full` | crystalball.io | Geopolitical intelligence, OSINT, defense | 37 |
| `tech` | tech.crystalball.io | Technology, AI/ML, startups, VC | 34 |
| `finance` | finance.crystalball.io | Markets, trading, macro, commodities | 29 |
...
export const STORAGE_KEYS = {
  panels: 'crystalball-panels', // Panel visibility toggles
  monitors: 'crystalball-monitors', // Monitor keyword configs
  mapLayers: 'crystalball-layers', // Map layer toggles
  disabledFeeds: 'crystalball-disabled-feeds', // Per-source feed disabling
} as const;
```
...
|-----|---------|------------|
| `crystalball-panel-spans` | Panel height/span sizes (1–4) | `Panel.ts` |
| `panel-order` | Drag-and-drop panel ordering | `App.ts` |
| `crystalball-variant` | Last-active variant (triggers reset on change) | `App.ts` |
| `crystalball-panel-order-v1.9` | Migration flag for v1.9 panel layout | `App.ts` |
...
Span values are persisted per-panel in the `crystalball-panel-spans` localStorage key as a JSON object `{ [panelId]: spanNumber }`.
...
## 4. Full Variant Panels (crystalball.io)
...
## 5. Tech Variant Panels (tech.crystalball.io)
...
## 6. Finance Variant Panels (finance.crystalball.io)
...
|---------|-----------------|--------|----------------|:--------------:|
| Panel visibility | `crystalball-panels` | `Record<string, PanelConfig>` JSON | `DEFAULT_PANELS` | ✅ |
| Panel ordering | `panel-order` | `string[]` JSON | Config declaration order | ✅ |
| Panel sizes/spans | `crystalball-panel-spans` | `Record<string, number>` JSON | All span-1 | ✅ |
| Map layer toggles | `crystalball-layers` | `MapLayers` JSON | `DEFAULT_MAP_LAYERS` | ✅ |
| Monitor keywords | `crystalball-monitors` | `Monitor[]` JSON | `[]` | ✅ |
| Disabled sources | `crystalball-disabled-feeds` | `string[]` JSON | `[]` | ✅ |
| Active variant | `crystalball-variant` | Plain string | `SITE_VARIANT` | ✅ |
| Banner dismissal | `banner-dismissed` (sessionStorage) | Timestamp string | — | Session only |

### 9.2 Variant Change Reset

When the stored variant (`crystalball-variant`) differs from the current `SITE_VARIANT`, the App constructor performs a full reset:
...
This ensures users switching between variant domains (e.g. from crystalball.io to tech.crystalball.io) get a clean default experience for the new variant.
...
Step-by-step guide to adding a new panel to Crystal Ball.
...
sequenceDiagram
 ...
 App->>localStorage: saveToStorage('crystalball-panels', panelSettings)
 ...
 App->>localStorage: loadFromStorage('crystalball-panels')
