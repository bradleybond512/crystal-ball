# Documentation Roadmap — Crystal Ball
...
- [ ] Nginx configuration (reference `deploy/nginx-crystalball.conf`)
- [ ] SystemD service setup (reference `deploy/crystalball-api.service`)
...
- [ ] Add entries for UI customizations on `feat/ui-customizations-crystalball` branch
