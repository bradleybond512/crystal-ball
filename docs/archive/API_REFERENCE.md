# Crystal Ball — API Reference

> Comprehensive reference for all Vercel Edge Function endpoints powering the Crystal Ball intelligence dashboard.

**Base URL**: All endpoints are relative to `/api/` (e.g., `https://crystalball.app/api/earthquakes`).
...
Crystal Ball exposes **60+ serverless endpoints** deployed as **Vercel Edge Functions** (unless noted otherwise). Every endpoint:
...
| # | Pattern | Matches |
|---|---------|---------|
| 1 | `crystalball\.app$` | `https://crystalball.app` |
| 2 | `\.crystalball\.app$` | `https://*.crystalball.app` |
...
Crystal Ball uses a **multi-tier caching strategy** to minimize upstream API calls, reduce latency, and stay within third-party rate limits.
