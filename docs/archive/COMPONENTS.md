# Component Documentation — Crystal Ball
...
Crystal Ball's UI is built entirely with **vanilla TypeScript** — no React, Vue, or
...
`localStorage['crystalball-panel-spans']` as a JSON map of `{ [id]: height }`.
...
Crystal Ball ships two independent map renderers and an adapter that picks the
...
Recent selections stored in `localStorage['crystalball_recent_searches']`
(most recent 10).
