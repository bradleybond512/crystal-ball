...
Status: UX-031 draft claim, approved for implementation September 15, 2026.
Discovery and design completed before implementation; Claude design review found
no blocking findings. The broader macOS plan remains in its separate plan branch.
...
UX-031 claims this bounded appearance-lifecycle scope. The user approved the
draft PR and implementation on September 15. Keep UX-025 and journey acceptance
separate. The tracker remains IN PROGRESS until the stated evidence exists.
...
Claude implementation refinements: test winning computed styles and both scrollbar
selector forms; use shared variant resolution; verify packaged event delivery
early. The design review is not a source-tip merge verdict.
