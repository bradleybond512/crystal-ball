...
This branch exists to do real selective integration of stale fork branches onto current `main` without touching the dirty working tree in `/Users/bradleybond/Developer/crystalball`.

## Canonical Branch State

- Integration branch: `codex/integration-main-20260329`
- Worktree path: `/Users/bradleybond/.codex/worktrees/crystalball/integration-main-20260329`
...
- Dirty primary workspace: `/Users/bradleybond/Developer/crystalball`
...
- No open PRs remain in `bradleybond512/crystal-ball`
...
- `0fce03ab` from `macos/claude/update-web-version-6BNZ1`
 ...
 - CORS allowlist for `bradleybond512` and `crystal-ball` Vercel deployments already exists
...
- `4e88fb60` `Revert "Align TypeScript deprecation gate with TS6"`
  - A clean `npm ci` install in `/tmp/crystalball-install-c1157df1` proved the lockfile still resolves `typescript@5.9.3`
...
- If you want to transplant it, inspect with:
  - `git -C /Users/bradleybond/Developer/crystalball show --stat 0399a33e`
...
```bash
cd /Users/bradleybond/.codex/worktrees/crystalball/integration-main-20260329
...
- `node_modules` had to be symlinked to `/Users/bradleybond/Developer/crystalball/node_modules` because sandboxed `npm ci` could not create the worktree-local directory
- `npm run typecheck:all` passed in the `.codex` worktree only when using the primary repo's dependency tree
- A clean `npm ci` in `/tmp/crystalball-install-c1157df1` resolved `typescript@5.9.3`
- That clean environment rejected `ignoreDeprecations: "6.0"`, so `e75c2f63` was reverted by `4e88fb60`
- `npm run build` reached Vite/Workbox and then failed with sandbox-style `EPERM` creating `/Users/bradleybond/.codex/worktrees/crystalball/integration-main-20260329/dist`
