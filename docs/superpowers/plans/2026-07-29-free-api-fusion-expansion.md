...
- [ ] Fixture test: EUR at 0.9200 vs 0.9210 → agree; EUR 0.92 vs 0.98 → disagreement surfaces. **CORRECTED 2026-07-30 — do NOT assert "names both providers"; that is unachievable.** `fuseObservations` names only the *outlier* in `disagreements[].providerIds`, never the consensus side. Verified by running the exact case through `ingestDomain`: the single row is `{ providerIds: ['er-api-fx'], value: 0.98, reason: 'Differs from consensus value 0.92.' }`, `confidenceMultiplier` 0.6. Assert instead: `disagreements.length >= 1`, `confidenceMultiplier <= 0.6`, and that BOTH provider ids appear in `fact.fingerprints` with distinct values. This correction applies to every disagreement fixture in this plan.
...
- **ACCEPTED LIMITATION (measured 2026-07-30, code-quality review): Frankfurter's freshness term is a constant 0, so the 12 h→5 d staleness band is invisible to fusion scoring.** Probed multiplier is a flat **0.732** for 16 h-, 40 h- and 88 h-old fixings. The proposed remedy — raising `frankfurter-fx.freshnessTtlMs` from 12 h to 24 h — was **REJECTED**: at 24 h the 40 h and 88 h cases still score 0, so it does not actually restore the band it claims to, while `provider-health.ts:76` measures the same constant against `lastSuccessAt` (the *fetch* time) and would therefore take twice as long to flag a genuinely dead Frankfurter. It also breaks the `provider-registry.test.mts:96` pin. A three-week-old fixing IS still caught — verified — but by the 5-day match window splitting the cluster (votes → 1), not by freshness. That is the intended detector for this domain; freshness is not.
