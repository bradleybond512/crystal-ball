...
Add ESLint with full type-checked rules, `eslint-plugin-unicorn`, and `eslint-plugin-sonarjs` to Crystal Ball. Covers both the TypeScript frontend (`src/`) and the Node.js sidecar + scripts (`src-tauri/sidecar/`, `scripts/`, `api/`). All existing violations are fixed upfront — no suppressions, no warnings (with one documented exception for the SSRF security guard).
