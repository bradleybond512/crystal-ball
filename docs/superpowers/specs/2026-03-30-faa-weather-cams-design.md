...
Integrate the FAA Aviation Weather Camera network into Crystal Ball as a four-phase feature: a panel, a DeckGL map layer, disaster-mode auto-surfacing, and AI-powered relevance scoring + image analysis. The primary lens is alert context — cameras are ranked and filtered by proximity to active NWS/GDACS alerts rather than by geography.
