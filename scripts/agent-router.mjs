...
const request = (requestIndex >= 0 ? args[requestIndex + 1] : args.join(' ')).toLowerCase();
...
const routes = [
  ...
  { agent: 'release_engineer', tests: ['npm run version:check', 'npm run lockfile:check'], high: true, words: ['release', 'sign', 'notar', 'updater', 'install', 'package', 'version'] },
...
const selected = routes.filter(route => route.words.some(word => request.includes(word)));
...
const tier = highAssurance ? 'high_assurance' : trivial ? 'mechanical' : selected.length > 1 ? 'standard' : 'focused';
