...
test('foreign ownership of the config is a blocker', () => {
  ...
  assert.match(report.findings[0].fix, /sudo chown -R 501:20/);
});
...
test('an unreadable config is reported once, not cascaded', () => {
  const probe = healthyProbe();
  probe.config.parseError = 'Unexpected end of JSON input';
  const report = buildClaudeAuthReport(probe);
...
test('stray scratch files are the only auto-fixable finding', () => {
  const probe = healthyProbe({ strayFiles: ['.claude.json.tmp1234', '.claude.json.lock'] });
  const report = buildClaudeAuthReport(probe);
  const fixable = autoFixableFindings(report);
  assert.equal(fixable.length, 1);
  assert.equal(fixable[0].id, 'config-stray-writes');
  assert.deepEqual(fixable[0].paths, ['.claude.json.tmp1234', '.claude.json.lock']);
});

test('no finding outside the stray-file rule is ever auto-fixable', () => {
  const probe = healthyProbe();
  probe.config.uid = 0;
  probe.config.writable = false;
