...
export class DailyWisdomPanel extends Panel {
 ...
 const storageKey = `crystalball-daily-wisdom-${today}`;
 try {
 const storedDate = localStorage.getItem('crystalball-daily-wisdom-date');
 if (storedDate === today) {
 ...
 try {
 localStorage.setItem('crystalball-daily-wisdom-date', today);
