...
export class HostageDiplomacyPanel extends Panel {
  ...
  private buildIndexBar(index: number, activeCount: number): HTMLElement {
    const color = index >= 80 ? 'var(--severity-critical, #ef4444)'
      : index >= 60           ? 'var(--severity-high,     #fb923c)'
      : index >= 40           ? 'var(--severity-medium,   #facc15)'
  ...
  private buildCountryScoresSection(scores: CountryScore[]): HTMLElement {
    ...
    for (const s of scores) {
      ...
      const color = s.score >= 80 ? 'var(--severity-critical, #ef4444)'
        : s.score >= 60           ? 'var(--severity-high,     #fb923c)'
        : s.score >= 40           ? 'var(--severity-medium,   #facc15)'
        :                           'var(--severity-low,      #4caf50)';
