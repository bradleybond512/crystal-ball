...
// Mode collapse: former per-mode quote affinity sets merged. Ghost keeps its
// own contemplative set; everything else is unconditional (union of war +
// disaster + finance tags) since those modes no longer trigger.
const DEFAULT_AFFINITY: ReadonlySet<string> = new Set([
  'Courage', 'Fortitude', 'Strength', 'Endurance', 'Steadiness', 'Resilience', 'Duty', 'Action',
  'Stillness', 'Trust', 'Presence', 'Comfort', 'Hope', 'Refuge', 'Rest', 'Acceptance', 'Peace', 'Calm',
  'Detachment', 'Perspective', 'Freedom', 'Wisdom', 'Simplicity', 'Priorities',
]);
...
export class InspirationQuotePanel extends Panel {
 ...
 const mode = getMode();
 const affinityTags = (mode ? MODE_TAG_AFFINITY[mode] : undefined) ?? DEFAULT_AFFINITY;
