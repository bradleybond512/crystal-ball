...
const REFRESH_MS = 60 * 60 * 1_000; // hourly — static data, no network calls
...
export class PandemicPreparednessPanel extends Panel {
  ...
  private refresh(): void {
    ...
    const indexCls =
      globalPreparednessIndex < 40
        ? 'read-critical'
        : globalPreparednessIndex < 60
          ? 'read-weak'
          : globalPreparednessIndex < 70
            ? 'read-adequate'
            : 'read-strong';
    ...
    this.setContent(
      ...
      outbreakSectionHtml +
      countrySectionHtml +
      '<div class="pp-footer">Sources: GHSI 2021 \xb7 WHO IHR \xb7 CDC \xb7 WHO Disease Outbreak News</div>' +
      '</div>',
    );
  }
