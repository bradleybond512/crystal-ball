...
function renderEncryptionThreatBanner(maxThreat: number, leadingCountry: string): string {
  ...
  let label = 'LOW';
  if (maxThreat >= 70) label = 'CRITICAL';
  else if (maxThreat >= 40) label = 'ELEVATED';
  else if (maxThreat >= 20) label = 'MODERATE';
  return `
...
function renderDominanceRankings(programs: QuantumProgram[]): string {
  const rows = programs.map((p, idx) => {
    ...
    const qubitStr = p.qubitCount == null ? 'N/A' : `${p.qubitCount.toLocaleString()} qubits`;
