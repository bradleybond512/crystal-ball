...
export class RegulatoryArbitragePanel extends Panel {
  constructor() {
    super('regulatory-arbitrage', 'Regulatory Arbitrage', REFRESH_MS);
  }
  ...
  protected override render(): void {
    ...
    replaceChildren(this.body, container);
  }
  ...
  private renderSummaryBar(s: ReturnType<typeof buildPanelSummary>): HTMLElement {
    ...
    const metric = (label: string, value: string, color?: string): HTMLElement => {
      ...
      el.append(h('span', { style: `font-size:16px;font-weight:700;${color ? `color:${color}` : ''}` }, value));
      return el;
    };
  ...
  private renderFatf(rows: FatfJurisdiction[]): HTMLElement {
    ...
    const statuses: Array<import('./regulatory-arbitrage-helpers').FatfStatus> = ['black', 'grey', 'monitored', 'compliant'];
