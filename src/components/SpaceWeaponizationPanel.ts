...
function renderProgramRow(p: SpaceWeaponProgram): string {
  ...
  return `<div style="display:flex;justify-content:space-between;align-items:center;padding:5px 8px;border:1px solid var(--border-subtle,#333);border-left:3px solid ${color};border-radius:3px;font-size:11px;gap:8px;">
    <div style="min-width:0;flex:1;">
      ...
      <div style="color:var(--text-secondary,#aaa);font-size:10px;">${escapeHtml(p.nation)} &middot; ${escapeHtml(p.category)} &middot; ${escapeHtml(p.developmentStage)}</div>
    </div>
    <div style="font-size:10px;font-weight:700;color:${color};text-transform:uppercase;letter-spacing:0.05em;">${escapeHtml(tier)} &middot; ${score}</div>
  </div>`;
}
...
function renderIncidentRow(inc: SpaceIncident): string {
  ...
  return `<div style="border:1px solid var(--border-subtle,#333);border-left:3px solid ${tierColor};border-radius:3px;padding:6px 8px;font-size:11px;">
    <div style="display:flex;justify-content:space-between;align-items:start;">
      <div style="font-weight:600;">${escapeHtml(inc.nation)} &middot; ${escapeHtml(inc.category)}</div>
      <div style="font-family:ui-monospace,monospace;color:var(--text-secondary,#aaa);font-size:10px;">${escapeHtml(inc.date)}</div>
    </div>
    <div style="margin-top:2px;color:var(--text-secondary,#aaa);font-size:10px;">${escapeHtml(inc.description)}${debrisNote}</div>
  </div>`;
}
...
export class SpaceWeaponizationPanel extends Panel {
  ...
  private renderSummary(data: ReturnType<typeof buildRenderData>): string {
    ...
    const catBadges = catEntries
      .map(
        ([cat, count]) =>
          `<span style="display:inline-block;padding:2px 8px;border:1px solid var(--border-subtle,#333);border-radius:8px;font-size:10px;margin-right:4px;margin-bottom:4px;">${escapeHtml(cat)} <strong>${count}</strong></span>`,
      )
      .join('');
    return `<div>
      ...
      <div style="display:flex;gap:16px;flex-wrap:wrap;font-size:11px;margin-bottom:8px;">
        <div>Leading nation: <strong>${escapeHtml(data.leadingNation)}</strong></div>
