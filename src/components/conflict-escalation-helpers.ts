...
export const WAR_RISK_WEIGHTS: Readonly<Record<Exclude<keyof WarRiskInput, 'deEscalationScore'>, number>> = {
  ...
  ceasefireFragilityScore: 0.2,
  intensityTrendScore: 0.2,
  escalationLadderScore: 0.25,
  crossDomainPressureScore: 0.1,
};
...
export function computeWarRiskIndex(input: WarRiskInput): WarRiskIndex {
  ...
  let topValue = 0;
  const keys = Object.keys(WAR_RISK_WEIGHTS) as Exclude<keyof WarRiskInput, 'deEscalationScore'>[];
  ...
  const deduction = Math.min(raw, deEscalation * 0.3);
