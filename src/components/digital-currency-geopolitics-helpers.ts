...
export function computeDollarHegemonyIndex(
  ...
  let trend: DedollarizationTrend = 'reversing';
  if (score < 40) trend = 'accelerating';
  else if (score < 60) trend = 'stable';

  let interpretation = 'USD hegemony significantly eroded — alternative infrastructure maturing.';
  if (score >= 70) interpretation = 'USD hegemony intact — challenges remain structural but not acute.';
  else if (score >= 50) interpretation = 'USD hegemony under moderate stress — diversification trends real but gradual.';
