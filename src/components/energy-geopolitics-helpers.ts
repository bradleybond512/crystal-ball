...
export interface LNGStressResult {
  ...
  majorSuppliers: Array<{ country: string; stressContribution: number }>;
...
const OPEC_MEMBERS: OPECMember[] = [
  { name: 'Saudi Arabia', code: 'SA', quotaMbpd: 9.0, actualMbpd: 9.0, status: 'compliant', complianceRate: 1.0, strategicAlignment: 'core' },
  ...
  { name: 'Kuwait', code: 'KW', quotaMbpd: 2.5, actualMbpd: 2.5, status: 'compliant', complianceRate: 1.0, strategicAlignment: 'core' },
  { name: 'Iran', code: 'IR', quotaMbpd: 3.0, actualMbpd: 3.2, status: 'suspended', complianceRate: 0.0, strategicAlignment: 'fringe' },
  { name: 'Kazakhstan', code: 'KZ', quotaMbpd: 1.6, actualMbpd: 1.8, status: 'over_producing', complianceRate: 0.78, strategicAlignment: 'fringe' },
  { name: 'Qatar', code: 'QA', quotaMbpd: 0.6, actualMbpd: 0.6, status: 'compliant', complianceRate: 1.0, strategicAlignment: 'core' },
];
...
export function calculateOPECCompliance(members: OPECMember[]): OPECCompliance {
  ...
  const cohesionTrend: OPECCompliance['cohesionTrend'] =
    cohesionScore >= 75 ? 'improving' : cohesionScore >= 55 ? 'stable' : 'deteriorating';
...
export function estimateLNGStress(params: {
  ...
  const stressLevel: LNGStressLevel =
    stressScore >= 75 ? 'crisis' : stressScore >= 50 ? 'stressed' : stressScore >= 25 ? 'elevated' : 'normal';
...
export function classifyWeaponizationRisk(params: {
  ...
  const tier: WeaponizationTier =
    score >= 70 ? 'extreme' : score >= 50 ? 'high' : score >= 25 ? 'moderate' : 'low';
