...
const MOCK_GROUPS: PMCGroup[] = [
  { id: 'wagner', name: 'Wagner Group / Africa Corps', sponsor: 'Russia', status: 'rebranded', estimatedStrength: 50000, activeTheaters: ['Ukraine', 'Mali', 'Libya', 'Sudan', 'CAR', 'Niger'], operationTypes: ['combat', 'training', 'intelligence'], humanRightsFlags: 9, revenueMUSD: 3500, governmentAffiliation: 95 },
  ...
  { id: 'msd', name: 'Mahad Al-Arabiya / MSD', sponsor: 'UAE', status: 'active', estimatedStrength: 15000, activeTheaters: ['Yemen', 'Libya', 'Somalia'], operationTypes: ['combat', 'training', 'security'], humanRightsFlags: 7, revenueMUSD: 1200, governmentAffiliation: 70 },
  ...
  { id: 'sinaloa-sec', name: 'Blackwater Successor (ACADEMI)', sponsor: 'USA', status: 'active', estimatedStrength: 20000, activeTheaters: ['Iraq', 'Afghanistan', 'UAE'], operationTypes: ['security', 'training', 'logistics'], humanRightsFlags: 3, revenueMUSD: 2000, governmentAffiliation: 30 },
  ...
  { id: 'ntc-lib', name: 'NTC Libyan Militias', sponsor: 'non-state', status: 'active', estimatedStrength: 25000, activeTheaters: ['Libya'], operationTypes: ['combat', 'security'], humanRightsFlags: 8, revenueMUSD: 400, governmentAffiliation: 20 },
];
...
export function rankBySponsor(groups: PMCGroup[]): Record<SponsorNation, PMCGroup[]> {
  ...
  for (const g of groups) { if (!out[g.sponsor]) out[g.sponsor] = []; out[g.sponsor].push(g); }
  return out;
}
...
export function getMostActiveTheater(groups: PMCGroup[]): string {
  ...
  for (const g of groups) for (const t of g.activeTheaters) counts[t] = (counts[t] || 0) + 1;
