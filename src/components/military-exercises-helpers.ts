...
export function getLargeExercises(exercises: MilitaryExercise[], minTroops = 10_000): MilitaryExercise[] {
...
export function getRecentExercises(exercises: MilitaryExercise[], afterYear: number): MilitaryExercise[] {
  return exercises.filter((e) => {
    const match = /\d{4}/.exec(e.date);
    if (!match) return false;
    return Number.parseInt(match[0]!, 10) >= afterYear;
  });
}
...
export function computeRegionalIntensity(exercises: MilitaryExercise[]): RegionalIntensity[] {
  ...
  return REGIONS.map((region) => {
    ...
    const troopScore = Math.min(100, (totalTroops / MAX_TROOPS) * 100) * 0.5;
