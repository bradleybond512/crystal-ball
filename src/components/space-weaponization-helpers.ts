...
export function scoreProgramThreat(p: SpaceWeaponProgram): number {
  const stageMult = { operational: 1.0, testing: 0.8, development: 0.5, conceptual: 0.2 }[p.developmentStage];
...
export function getMostAdvancedNation(programs: SpaceWeaponProgram[]): SpacePowerNation {
  ...
  return nations.sort((a, b) => getNationCapabilityScore(programs, b) - getNationCapabilityScore(programs, a))[0];
}
