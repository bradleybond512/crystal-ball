...
export interface GlobalConflictEconomyIndex {
  ...
  topRegions: Array<{ region: ConflictRegion; totalBillions: number }>;
  /** Resource types appearing across the most profiles, descending. */
  dominantResourceTypes: Array<{ type: ResourceType; profileCount: number }>;
...
export const CONFLICT_ECONOMY_PROFILES: readonly ConflictEconomyProfile[] = [
  ...
  },
  {
    ...
    annualRevenueMinBillions: 1.0,
    annualRevenueMaxBillions: 2.0,
  ...
  },
  {
    ...
    annualRevenueMinBillions: 1.0,
    annualRevenueMaxBillions: 3.0,
    annualRevenueMidBillions: 2.0,
    keyNote:
...
export function revenueClass(annualRevenueBillions: number): RevenueClass {
  ...
  if (annualRevenueBillions < 1.0)  return 'major';
  return 'mega';
}
...
export function buildRenderData(
  ...
  const rows: ConflictEconomyRenderRow[] = profiles.map((profile) => {
    ...
    return {
      ...
      resourceLabels: profile.primaryRevenueSources.map(resourceTypeLabel),
