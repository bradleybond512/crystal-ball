...
export interface TuningApplyDeps {
   ...
   *  (conservative): the gate then holds tunings for approval. Wiring the
   *  replay-harness / backtest-engine here is what flips auto-apply on. */
...
export function runTuningApply(deps: TuningApplyDeps = {}): TuningApplyResult {
  ...
  for (const p of proposals) {
    ...
    if (gated.verdict.decision === 'allow_auto') {
      ...
      console.warn(
        `[tuning] auto-applied ${p.algorithmId}.${p.parameterId}: ${p.priorValue} → ${p.nextValue}`
        + ` | ${p.rationale}`
        + ` | revert: setTunedParam('${p.algorithmId}','${p.parameterId}',${p.priorValue})`,
      );
    } else {
      heldForApproval += 1;
    }
