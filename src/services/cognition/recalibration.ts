...
export function recalibrate(p: number, curve: ReliabilityCurve): RecalibrationResult {
  ...
  const explanation: string = adjustment === 0 ? `${domainLabel} at ~${pctRaw}% are well-calibrated (n=${binN}, observed ${pctObs}%) — no adjustment` : `${domainLabel} at ~${pctRaw}% have materialized ${pctObs}% of the time (n=${binN}) → adjusted to ${pctAdj}%`;
