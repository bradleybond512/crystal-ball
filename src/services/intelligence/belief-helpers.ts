...
export function propagateConfidence(beliefs: BeliefValue[], rule: CombiningRule): BeliefValue {
  ...
  switch (rule) {
    ...
    case 'max':
      ...
      break;
    case 'log-odds':
      point = sigmoid(sum(points.map(logit)));
      lower = sigmoid(sum(lowers.map(logit)));
      upper = sigmoid(sum(uppers.map(logit)));
      break;
    case 'average':
    default:
      point = mean(points);
...
export function getProbabilityLabel(point: number): ProbabilityLabel {
  const p = clamp01(point);
  if (p < 0.10) return 'almost-certainly-not';
  if (p < 0.30) return 'very-unlikely';
...
export function formatBelief(belief: BeliefValue): string {
  ...
  return `${getProbabilityLabel(belief.point)} (${pct(belief.point)}%, CI ${pct(belief.lower)}–${pct(belief.upper)}%)`;
}
