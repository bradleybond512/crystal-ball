...
export const IDB_BACKED_STORE_KEYS: readonly string[] = [
  ...
  // Additional keys discovered in the w5 IDB audit: each can grow to hundreds
  // of KB and all are first read after installIdbStorageRouting() executes
  // (panelLayout.init fires after preload+routing in App.ts).
  'wm-threat-synthesis-v1',          // ThreatReport JSON, up to ~150 entries
  'wm-intelligence-briefing-v1',     // IntelligenceBriefing with 10–15 items slices
  'wm-survival-advice-v1',           // SurvivalAdvice, 15-min TTL; re-derived from Claude
  'crystalball-forecast-calibration-v1', // up to 500 PredictionRecord entries
];
...
async function flushPending(): Promise<void> {
  ...
  // Snapshot the batch and clear ONLY the keys we're about to attempt — not
  // the entire map. New writes that arrive during the async loop get their own
  // entry in `pending` and will be flushed on the next scheduled timer. Keys
  // whose writes fail are re-queued so the next flush retries them; the
  // in-memory mirror stays authoritative this session regardless.
  ...
  for (const [key, value] of batch) {
    try {
      ...
      // Re-queue only if a newer write hasn't superseded this value.
      if (!pending.has(key)) {
