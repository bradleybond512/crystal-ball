...
function getCacheDb(): Promise<IDBDatabase> {
  ...
  cacheDbPromise = new Promise((resolve, reject) => {
    ...
    // A stale open connection (another tab) is blocking this version open.
    request.addEventListener('blocked', () => reject(new Error('[persistent-cache] IndexedDB open blocked')));

    request.addEventListener('success', () => {
      const db = request.result;
      db.addEventListener('close', () => { cacheDbPromise = null; });
      // If a future schema version is opened in another tab, close this
      // connection so the upgrade isn't blocked. The next getCacheDb() call
  ...
  // Clear the in-flight promise on rejection so the next call retries rather
  // than returning a permanently-rejected promise (matching the pattern in
  // alert-store, storage.ts, and reasoning-memory).
