...
let _currentMode: AppMode | null = null;
...
function _playModeSound(mode: AppMode | null): void {
  if (isMuted()) return;
  // Only Ghost retains an activation chime. Former finance/war/peace/disaster
  // chimes were pure mode-transition stings and were deleted per the mode
  // collapse. God's Vision is an immersive view, not an audible event.
  if (mode === 'ghost') _playGhostActivate();
}
...
function _onModeChangedLayers(mode: AppMode | null, prev: AppMode | null): void {
  if (isMuted()) return;

  // Radar + stock-ticker ambient layers used to gate on war/finance modes.
  // With mode collapse, they now run unconditionally (on in the default
  // state) and are only toggled by the user via spatial-layer preferences.
  if (isSpatialLayerEnabled('radar')) _scheduleRadar(); else _stopRadar();
  if (isSpatialLayerEnabled('ticker')) _scheduleTicker(); else _cancelTicker();

  // Ghost drone remains gated on Ghost mode only.
