...
import { deduplicateHeadlines } from '../server/crystalball/news/v1/dedup.mjs';
...
describe('getHumanitarianSummary handler', () => {
  const src = readSrc('server/crystalball/conflict/v1/get-humanitarian-summary.ts');
...
describe('humanitarian_summary.proto', () => {
  const proto = readSrc('proto/crystalball/conflict/v1/humanitarian_summary.proto');
...
describe('LLM prompt political context (LOW-1)', () => {
  const src = readSrc('server/crystalball/news/v1/_shared.ts');
...
describe('getVesselSnapshot caching (HIGH-1)', () => {
  const src = readSrc('server/crystalball/maritime/v1/get-vessel-snapshot.ts');
