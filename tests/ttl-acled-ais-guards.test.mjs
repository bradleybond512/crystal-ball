...
describe('cache TTL alignment with upstream refresh rates', () => {
 ...
 const src = readSrc('server/crystalball/climate/v1/list-climate-anomalies.ts');
 ...
 const src = readSrc('server/crystalball/wildfire/v1/list-fire-detections.ts');
...
describe('ACLED consumers use shared cache layer', () => {
 ...
 const src = readSrc('server/crystalball/conflict/v1/list-acled-events.ts');
 ...
 const src = readSrc('server/crystalball/unrest/v1/list-unrest-events.ts');
 assert.match(src, /fetchAcledCached/,
 'Unrest handler should use shared ACLED fetch');
  });

  it('risk scores handler imports fetchAcledCached', () => {
 const src = readSrc('server/crystalball/intelligence/v1/get-risk-scores.ts');
 assert.match(src, /fetchAcledCached/,
 'Risk scores handler should use shared ACLED fetch');
  });

  it('no handler has its own ACLED_API_URL constant', () => {
 const conflict = readSrc('server/crystalball/conflict/v1/list-acled-events.ts');
 const unrest = readSrc('server/crystalball/unrest/v1/list-unrest-events.ts');
 const riskScores = readSrc('server/crystalball/intelligence/v1/get-risk-scores.ts');
